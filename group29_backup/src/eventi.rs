...
/* Versione senza premere il mouse
   Check versione ->  controlla solo se tocca la parte superiore,inferiore e laterale e non la forma del tracciato
    ...
    // Funzione per tracciare i punti del mouse in continuo
    pub fn handle_mouse_events() {
        ...
        if let Err(error) = listen(move |event| {
            match event.event_type {
                EventType::MouseMove { x, y } => {
                    ...
                    // Controlla se i punti formano un rettangolo lungo i bordi del monitor
                    if is_rectangle_along_screen_borders(&points, screen_width, screen_height) {
                        ...
                        points.clear(); // Resetta i punti dopo aver riconosciuto un rettangolo
                    }
                }
                _ => (),
            }
